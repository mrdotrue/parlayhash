...
ABSL_FLAG(
    int, u, -1,
    "Percent of operations that are updates (-1 will use multiple percents)");
ABSL_FLAG(double, z, -1.0,
          "Zipfian parameter (-1 will use multiple parameters)");
...
generate_integer_distribution(
    long n, // num entries in map
    long p,
    double zipfian_param) // zipfian parameter [0:1) (0 is uniform, .99 is high
                          // skew)
{
  // total samples used
  long m = 10 * n + 1000 * p;
...
test_loop(const std::string &command_name, std::string info,
          const parlay::sequence<typename Map::K> &a,
          const parlay::sequence<typename Map::K> &b,
          long p,             // num threads
          long rounds,        // num trials
          int update_percent, // percent of operations that are either insert or
                              // delete (1/2 each)
  ...
  for (int i = 0; i < rounds + warmup; i++) {
    {
          ...
          [&](size_t i) {
            ...
            while (true) {
              ...
              if (++j >= (i + 1) * mp)
                j = i * mp;
              if (++k >= (i + 1) * mp)
                k = i * mp + 1; // offset so different ops on different rounds
              cnt++;
              total++;
            }
          },
      ...
      std::cout << command_name << "," << "update=" << update_percent << "%,"
                << "n=" << n << "," << "p=" << p << "," << info << ","
                << "grow=" << grow << "," << "mem_pe=" << (int)bytes_pe << ","
...
#ifdef USE_SET
template <typename K_, typename Hash> struct bench_set {
  using K = K_;
...
int main(int argc, char *argv[]) {
  ...
  double latency_cuttoff =
      P.getOptionDoubleValue("-latency", 0.0); // in miliseconds
  ...
  if (n != 0)
    sizes = std::vector<long>{n};
  if (update_percent != -1)
    percents = std::vector<int>{update_percent};
  if (zipfian_param != -1.0)
    zipfians = std::vector<double>{zipfian_param};
  ...
  if (!string_only) {
    ...
    for (auto zipfian_param : zipfians)
      for (auto update_percent : percents) {
        for (auto n : sizes) {
          auto [a, b] =
              generate_integer_distribution<int_type>(n, p, zipfian_param);
          std::stringstream str;
          str << "long_long,z=" << zipfian_param;
          auto [itime, btime, size, q_latency, u_latency] =
              test_loop<int_map_type>(command_name, str.str(), a, b, p, rounds,
                                      update_percent, upsert, trial_time,
                                      latency_cuttoff, verbose, warmup, grow,
                                      expand);
...

    {
      ...
      for (auto n : sizes) {
        auto [a, b] =
            generate_integer_distribution<small_int_type>(n, p, zipfian_param);
        std::stringstream str;
  ...
  if (!no_string) { // && n == 0 && update_percent == -1 && zipfian_param ==
                    // -1.0) {
    int cnt = 0;
    for (auto update_percent : percents) {
      long n = 20000000;
  ...
  if (print_means) {
    if (latency_cuttoff > 0) {
      std::cout << "query latency = " << geometric_mean(q_latencies) << "%@"
                << latency_cuttoff << "usecs" << std::endl;
      std::cout << "update latency = " << geometric_mean(u_latencies) << "%@"
                << latency_cuttoff << "usecs" << std::endl;
    } else {
      std::cout << "initial insert geometric mean of mops = "
                << geometric_mean(insert_times) << std::endl;
      std::cout << "benchmark geometric mean of mops = "
                << geometric_mean(bench_times) << std::endl;
#ifdef JEMALLOC
      std::cout << "bytes/element geometric mean = "
